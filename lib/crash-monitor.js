...
class CrashMonitor extends EventEmitter {
  ...
  _parseLogTimestamp(line) {
    const m = line && line.match(/^(\d{2})-(\d{2})\s+(\d{2}):(\d{2}):(\d{2})\.(\d+)/);
    if (!m) return 0;
    const now = new Date();
  ...
  _isStaleCrash(lines) {
    if (!this.monitorStartedAt) return false;
    for (const l of lines) {
      const ts = this._parseLogTimestamp(l);
      if (ts > 0) return ts < this.monitorStartedAt - 5000;
    }
    return false;
  }
  ...
  _execForSerial(serial, args) {
    return this._exec(['-s', serial, ...args]).catch((e) => {
      const msg = String(e && e.message || e);
      // 디바이스가 offline / unauthorized / not found 인 일시적 상태는 빈 문자열로 처리.
      // (모니터 폴링/와치독이 unhandled rejection 으로 새는 것을 방지)
      if (/device offline|device .* not found|device unauthorized|no devices\/emulators/i.test(msg)) {
        return '';
      }
      throw e;
    });
  ...
  async _finishStreamCrash(serial, state) {
    ...
    if (this._isStaleCrash(state.lines)) return;
  ...
  _startPollTimer() {
    ...
    this.pollTimer = setInterval(() => {
      this._pollAllDevices().catch((e) => console.warn('[crash-monitor] poll failed:', e.message));
    }, this.pollInterval);
    setTimeout(() => {
      this._pollAllDevices().catch((e) => console.warn('[crash-monitor] initial poll failed:', e.message));
    }, 1000);
  }
  ...
  async _pollCrashBuffer(serial) {
    ...
    while (i < lines.length) {
      ...
      const matchType = /FATAL EXCEPTION/i.test(line) ? 'CRASH'
        : /FATAL signal/i.test(line) ? 'NATIVE_CRASH'
        : /ANR in/i.test(line) ? 'ANR'
        : null;
      if (matchType) {
        const block = this._extractBlock(lines, i, matchType);
        const app = this._extractApp(block.lines);
        if (app && this._isRelevantApp(app) && !this._isStaleCrash(block.lines)) {
          const key = this._makeCrashKey(serial, matchType, app, block.lines);
          if (!this.seenCrashKeys.has(key)) {
            this.seenCrashKeys.add(key);
            await this._emitCrash(serial, matchType, app, block.lines.join('\n'));
          }
        }
        i = block.endIdx;
        continue;
  ...
  _startWatchdog() {
    ...
    this.watchdogTimer = setInterval(() => {
      this._watchdogTick().catch((e) => console.warn('[crash-monitor] watchdog tick failed:', e.message));
  ...
  _looksLikeNormalExit(contextLines) {
    const text = contextLines.join('\n');
    if (/exited due to signal 9.*Killed/i.test(text)) return true;
    if (/onTrimMemory.*?level=(BACKGROUND|COMPLETE|MODERATE|RUNNING_LOW|RUNNING_CRITICAL)/i.test(text)) return true;
    if (/am_kill|lowmemorykiller|am_proc_died/i.test(text)) return true;
    if (/Force stopping|Killing\s+\d+:/i.test(text)) return true;
    if (/Process .* has died/i.test(text)) return true;
    if (/onPause|onStop/i.test(text)) return true;
    if (/UID_REMOVED|Stopping service/i.test(text)) return true;
    // 추가 시그니처 (백그라운드 정리 / 시스템 종료)
    if (/Scheduling restart of crashed service/i.test(text)) return true;
    if (/cleanUpApplicationRecord|killProcessGroup/i.test(text)) return true;
    if (/app_died|procstate change/i.test(text)) return true;
    if (/oom_score_adj|adj=\d+/i.test(text)) return true;
    return false;
  }

  // 종료 시점에 우리 앱이 포그라운드였는지 확인.
  // 아니면(런처 / 다른 앱이 떠있음) → 사용자 의도로 빠져나간 백그라운드 종료로 간주.
  async _isAppForeground(serial, pkg) {
    try {
      const out = await this._execForSerial(serial, ['shell', 'dumpsys', 'window', 'displays']);
      const m = out && out.match(/mCurrentFocus=[^\n]*\s([\w.]+)\//);
      const focusedPkg = m ? m[1] : '';
      if (!focusedPkg) {
        // 화면 꺼짐 등으로 focus 없으면 mFocusedApp 으로 폴백
        const m2 = out && out.match(/mFocusedApp=[^\n]*\s([\w.]+)\//);
        return m2 ? m2[1] === pkg : false;
      }
      return focusedPkg === pkg;
    } catch {
      // 조회 실패 시 보수적으로 true (= 기존 로직대로 진행)
      return true;
    }
  }
  ...
  async _watchdogCheckDevice(serial) {
    ...
    if (prevPid && !currentPid) {
      // 1차 false-positive 방지: 잠깐 후 재확인 (ADB 명령이 일시적으로 빈 응답 줄 때 흔함)
      await new Promise((r) => setTimeout(r, 1500));
      const recheckPid = await this._getPid(serial, pkg);
      if (recheckPid) {
        this.watchedPids.set(serial, recheckPid);
        return;
      }
      // 다른 OVERDARE 패키지(.dev / release) 가 살아있으면 사용자가 단순 전환한 것
      const otherDetected = await this._autoDetectPkg(serial);
      if (otherDetected) {
        this.watchedPids.set(serial, otherDetected.pid);
        return;
      }

      this.watchedPids.set(serial, null);
      const buf = this.deviceBuffers.get(serial) || [];
      const recent = buf.slice(-200);

      const hasFatal = recent.some((l) => /FATAL EXCEPTION|FATAL signal|ANR in/i.test(l));

      // 포그라운드 검증: FATAL 시그니처가 없을 때만 적용 (진짜 크래시는 포그라운드 여부 무관)
      if (!hasFatal) {
        const wasForeground = await this._isAppForeground(serial, pkg);
        if (!wasForeground) {
          // 다른 앱/런처가 떠있음 → 사용자가 빠져나간 정상 종료로 간주
          return;
        }
        if (this._looksLikeNormalExit(recent)) return;
      }
